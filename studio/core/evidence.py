...
class EvidenceSource:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.name, str) or not self.name.strip():
            raise ValueError("EvidenceSource.name must be a non-empty string.")
        ...
        if self.reference is not None and not isinstance(self.reference, str):
            raise TypeError("EvidenceSource.reference must be a string or None.")

        if self.metadata is not None and not isinstance(self.metadata, str):
            raise TypeError("EvidenceSource.metadata must be a string or None.")
...
class Claim:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.supporting_evidence, list):
            raise TypeError("Claim.supporting_evidence must be a list.")

        if not isinstance(self.counter_evidence, list):
            raise TypeError("Claim.counter_evidence must be a list.")

        for evidence in self.supporting_evidence:
            if not isinstance(evidence, Evidence):
                raise TypeError(
                    "Claim.supporting_evidence must contain Evidence objects."
                )

        for evidence in self.counter_evidence:
            if not isinstance(evidence, Evidence):
                raise TypeError(
                    "Claim.counter_evidence must contain Evidence objects."
                )

        _validate_confidence(self.confidence, "Claim.confidence")
        self.confidence = float(self.confidence)

        if self.uncertainty is not None and not isinstance(
            self.uncertainty, str
        ):
            raise TypeError("Claim.uncertainty must be a string or None.")